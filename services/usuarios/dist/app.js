...
async function bootstrap() {
    ...
    server.setErrorHandler((error, _req, reply) => {
        ...
        if (error.validation) {
            return reply.code(400).send(fail(400, 'SxUS', error.message, 'Validation Error'));
        }
        server.log.error(error);
        return reply.code(code).send(fail(code, 'SxUS', code === 500 ? 'Error interno del servidor' : error.message));
    });
    // ── Health check ───────────────────────────────────────────────────
