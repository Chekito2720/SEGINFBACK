...
import { registroSchema, loginSchema, getPerfilSchema, updatePerfilSchema, updatePermisosSchema, deleteUsuarioSchema, } from '../schemas/usuario.schema.js';
const usuariosRoutes = async (fastify) => {
    const svc = new UsuarioService(fastify.db, Number(process.env.BCRYPT_ROUNDS ?? 10));
    ...
    fastify.post('/registro', { schema: registroSchema }, async (req, reply) => {
    ...
    fastify.get('/me', {}, async (req, reply) => {
        ...
        if (!id)
            return reply.code(401).send(fail(401, 'SxUS', 'No autenticado', 'Unauthorized'));
    ...
    /** GET /usuarios/:id */
    fastify.get('/:id', { schema: getPerfilSchema }, async (req, reply) => {
        const usuario = await svc.obtenerPorId(req.params.id);
        return reply.send(ok(200, 'SxUS', [usuario]));
    });
    /** PATCH /usuarios/:id */
    fastify.patch('/:id', { schema: updatePerfilSchema }, async (req, reply) => {
    ...
    fastify.put('/:id/permisos', { schema: updatePermisosSchema }, async (req, reply) => {
    ...
    /** DELETE /usuarios/:id */
    fastify.delete('/:id', { schema: deleteUsuarioSchema }, async (req, reply) => {
